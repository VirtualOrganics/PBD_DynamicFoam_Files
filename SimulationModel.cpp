...
void SimulationModel::reset()
{
	...
	// currents
	for (unsigned int i = 0; i < m_currents.size(); i++)
	{
	}
...
CurrentData & SimulationModel::getCurrents()
{
	return m_currents;
}
...
{
	// fprintf (stderr,"updateConstraints\n");
	for (unsigned int i = 0; i < m_constraints.size(); i++) {
	...
	const TriangleModel::ParticleMesh::UVs& uvs)
{
	...
	for (unsigned int i = 0; i < (2*nPoints)-2 ; i++) {
		m_currents.addVertex();
	}

	// fprintf (stderr,"addTriangleModel %d %d\n",m_particles.size(),m_currents.size());
