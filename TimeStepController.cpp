...
TimeStepController::TimeStepController() 
{
	...
	m_gravity = Vector3r(0.0, 0.0, 0.0);
}
...
void TimeStepController::step(SimulationModel &model)
{
	...
	/*
	fprintf (stderr,"step: numBodies = %d\n",numBodies);
	fprintf (stderr,"step: pd.size = %d\n",(int)pd.size());
	fprintf (stderr,"step: od.size = %d\n",(int)od.size());
	*/
...
void TimeStepController::positionConstraintProjection(SimulationModel &model)
{
	...
	// fprintf (stderr,"positionConstraintProjection groups.size() %d\n",groups.size());
	while (iter < m_maxIter)
	{
		for (unsigned int group = 0; group < groups.size(); group++)
		{
			const int groupSize = (int)groups[group].size();
			// fprintf (stderr,"positionConstraintProjection groupSize %d %d\n",group,groupSize);
			#pragma omp parallel if(groupSize > MIN_PARALLEL_SIZE) default(shared)
			{
				...
				for (int i = 0; i < groupSize; i++)
				{
					...
					// fprintf (stderr,"positionConstraintProjection constraintIndex %d\n",constraintIndex);

					// fprintf (stderr,"calling updateConstraint\n");
					constraints[constraintIndex]->updateConstraint(model);
					// fprintf (stderr,"calling solvePositionConstraint\n");
...
void TimeStepController::velocityConstraintProjection(SimulationModel &model)
{
	...
	// fprintf (stderr,"velocityConstraintProjection: %d\n",groups.size());
	for (unsigned int group = 0; group < groups.size(); group++)
	{
		const int groupSize = (int)groups[group].size();
		// fprintf (stderr,"\t%d %d\n",group,groupSize);
