	...
	Eigen::Matrix<Real, 1, 1> &constraintValue)
{
	...
	// C(0, 0) = (x[2] - x[0]).norm() - restLength;
	// C(0, 0) = (x[1] - x[2]).norm() - restLength;
	constraintValue = C;
}
...
bool GenericDistanceConstraint::initConstraint(SimulationModel &model, const unsigned int particle1, const unsigned int particle2)
{
	...
	// fprintf (stderr,"m_restLength ----> %.1f %.1f %.1f %.1f %f ",x1_0[0],x1_0[2],x2_0[0],x2_0[2],(x2_0 - x1_0).norm());
	// m_restLength = (x2_0 - x1_0).norm();
	// m_restLength = (x2_0 - x1_0).norm() * 0.5;
	// m_restLength = (x2_0 - x1_0).norm() * 1.5;
	// m_restLength = (x2_0 - x1_0).norm() * 0.9;
	// m_restLength = (x2_0 - x1_0).norm() * 0.75;
	rad1 = pd.getRadius(particle1);
	rad2 = pd.getRadius(particle2);
	// m_restLength = (rad1 + rad2) * 0.8;
	// fprintf (stderr,"restlength = %f ",m_restLength);
	m_restLength = (rad1 + rad2) * 0.9;
	// m_restLength = (rad1 + rad2) * 2.0;
	// fprintf (stderr,"%f\n",m_restLength);
...
bool GenericDistanceConstraint::solvePositionConstraint(SimulationModel &model)
{
	...
	// fprintf (stderr,"this one: solvePositionConstraint %d %d\n",i1,i2);
	...
	rad1 = pd.getRadius(i1);
	rad2 = pd.getRadius(i2);
	// m_restLength = (rad1 + rad2) * 0.8;
	// fprintf (stderr,"solvePositionConstraint %d %d %f %f %f ",i1,i2,rad1,rad2,m_restLength);
	m_restLength = (rad1 + rad2) * 0.9;
	// m_restLength = (rad1 + rad2) * 2.0;
	// fprintf (stderr,"%f\n",m_restLength);
